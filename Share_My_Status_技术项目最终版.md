...
5. 飞书链接预览：通过**官方 SDK 长链接会话**订阅并监听**链接预览拉取事件**，解析 Sharing Key → 读取最新状态与活动摘要 → 返回预览内容用于签名展示；当状态变化且命中刷新策略时，触发**预览更新能力**进行刷新（遵循平台的幂等与频控约束）。
...
### 3.1 链接定制与模板渲染
公开分享链接格式：`https://lexample.com/x/{SharingKey}?r=&m=`。
...
#### 3.1.6 聚合统计
> 注意：以下变量需用户**显式授权**历史存储与汇总；来源为统计表。
> 
...
- `m`（mark）作为**轻量文本模板**，支持 `{artist}{title}{album}` 三个占位符，并**新增支持布尔变量 charging 的三元表达式**；示例：`{artist}的{title} — {charging?'充电中':'未在冲电'}`。**默认模板**仍为“`{artist}的{title}`”。当某字段缺失时，自动删除该占位符；`charging` 缺失时**按默认视为 false**处理。

- **模板规则限制**：仅允许使用**本表列出的变量**与**基本条件表达式**；不支持复杂逻辑、函数调用或嵌套表达式。
...
- 自动解析
	- 提取 `SharingKey`（从路径段 `x/{SharingKey}`）。
	- `r` 参数：必须以 `http://` 或 `https://` 开头；进行**协议校验**、**域名白名单校验**、**HTML/XSS 转义**；非法时给出红色提示并禁用“生成链接”。
	- `m` 参数：仅支持 `{artist}{title}{album}` 占位符；默认模板为“{artist}的{title}”；支持布尔变量的三元表达式 `{charging?'充电中':'未在冲电'}`，当 `charging` 缺失时视为 `false`。

- 可视化拼接与实时预览
	- 所见即所得地插入变量；预览面板以当前状态快照渲染文本。
	- 校验与提示：**长度建议 ≤256**、统一 **HTML/XSS 转义**、字符集建议为中英文/数字/常见标点；超限时显示截断提示。
	- 一键复制：点击“复制最终链接”将**规范化后的 URL**放入剪贴板。

- 与本地设置联动
	- **白名单**：变量渲染仅来自被允许的采集源（如仅 QQ 音乐）；未允许的源将显示“未采集”。
	- **隐私开关**：当用户关闭某类采集（如活动），对应变量自动置空或不渲染。
	- **错误提示与兜底**：解析失败或预览不可用时，降级为默认模板“{artist}的{title}”；当无音乐播放时显示“未在播放”。

> **客户端链接定制安全与兜底**：
> - 所有用户输入（r/m）统一做 **协议校验 + 白名单校验 + HTML/XSS 转义**。
> - `charging` 三元表达式仅允许简单条件；不支持函数与嵌套逻辑。
...
- 飞书事件处理：在**官方 SDK 长链接会话**内接收**链接预览拉取事件**，内部 handler 解析请求并执行预览渲染与必要的**预览更新能力**，遵循幂等与频控。
