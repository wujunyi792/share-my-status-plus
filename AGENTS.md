...
Share My Status Plus — a real-time personal status sharing platform (Go backend + React frontend + PostgreSQL + Redis). See `README.md` for architecture and `README.dev.md` for local dev commands.

### Services

| Service | How to start | Port |
|---|---|---|
| PostgreSQL + Redis | `sudo docker compose up -d postgres redis` (from repo root) | 5432 / 6379 |
| Go Backend | `cd backend && APP_ENV=debug go run .` | 8080 |
| React Frontend | `cd frontend && pnpm dev` | 3000 |

### Non-obvious caveats

- **Go version**: The project requires Go 1.26 (`go.mod`). The VM snapshot installs it to `/usr/local/go`; the PATH is set in `~/.bashrc`.
- **Backend env file**: The backend loads `.env.debug` when `APP_ENV=debug`. The file must point PostgreSQL/Redis to `localhost` (not the Docker service name `postgres`/`redis`) because the backend runs outside Docker.
