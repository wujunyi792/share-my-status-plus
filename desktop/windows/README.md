...
- **首次运行引导**：未配置时自动弹出设置窗口；并有一次性气泡提示托盘图标位置（Windows 会把
  新图标折叠到 ▲ 溢出区，避免用户找不到）。
- **保存即上报**：在设置里点「保存并上报」，配置有效时自动开始上报，无需再去找开关；
  之后可在系统托盘右键图标随时开始 / 停止，托盘左键单击 / 双击 / 点气泡都可打开设置。
- **配置粘贴/复制**：除文件导入导出外，支持「粘贴配置」（弹输入框、自动填入剪贴板内容）与
  「复制配置」到剪贴板（可选不含 Secret Key），对齐 macOS 的剪贴板配置体验。
...
- **自动更新**：基于 **Velopack** 的静默更新（Windows 上的 Sparkle 等价物）。启动后台静默
  检查 GitHub Releases，发现新版本时托盘菜单变为「更新到 vX.Y」并弹气泡；确认后后台下载
  （支持增量包）并重启到新版本。开发/绿色 exe（非安装态）会回退为打开 Releases 手动下载。
- **快捷入口**：托盘菜单可「打开我的状态页 / 自定义飞书签名」（由服务端 `client/resources`
  返回的链接驱动）、「检查更新」、「关于」、「打开日志文件夹」。
...
`dotnet run` 时应用以非安装态运行，自动更新会回退为手动（这是预期的）。
...
注意：Velopack 打包**不要**用 `PublishSingleFile`（它需要常规发布目录）。
...
- **代码签名**：当前未做 Authenticode 签名（首次运行会有 SmartScreen 提示）。
  后续可像 macOS 那样在 CI 注入证书 secret，并给 `vpk pack` 传 `--signParams` 对安装器与
  增量包签名。
